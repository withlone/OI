...
void build_edge(){
	S = 1 ; T = n * m ;
	For( i , 1 , n )For( j , 1 , m - 1 )
		Ins( num[i][j] , num[i][j + 1] , road1[i][j] ) ;
	For( i , 1 , n - 1 )For( j , 1 , m )
		Ins( num[i][j] , num[i + 1][j] , road2[i][j] ) ;
	For( i , 1 , n - 1 )For( j , 1 , m - 1 )
		Ins( num[i][j] , num[i + 1][j + 1] , road3[i][j] ) ;
}
...
void dinic(){
	int res = 0 , t;
	while( bfs() ){
		memcpy( cur , Ht , sizeof( int ) * ( n * m + 1 ) );
